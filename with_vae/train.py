...
class CachedCOCODataset(Dataset):
    def __init__(self, cache_dir):
        ...
        and CLIP embeddings directly from disk.
        """
        self.cache_dir = cache_dir
        
        # Gather all latent files and sort them to maintain alignment
        self.latent_files = sorted([
            f for f in os.listdir(cache_dir) if f.startswith("latent_") and f.endswith(".pt")
        ])
    ...
    def __getitem__(self, idx):
        latent_filename = self.latent_files[idx]
        
        # Reconstruct the matching CLIP filename (e.g., latent_000012.pt -> clip_000012.pt)
        clip_filename = latent_filename.replace("latent_", "clip_")
        
        latent_path = os.path.join(self.cache_dir, latent_filename)
...
def get_loader(root = "train2017",annFile = "annotations/annotations/captions_train2017.json"):
    ...
    dataset = CachedCOCODataset(cache_dir="./coco_cached_features")
